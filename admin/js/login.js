if (new URLSearchParams(window.location.search).get('error') === 'acces_refuse') {
  document.getElementById('errorBox').classList.remove('hidden')
}

;(function() {
  const C = window.ADMIN_CONFIG || {}
  const n = document.getElementById('cfg-nom');   if (n) n.textContent = C.NOM   || ''
  const v = document.getElementById('cfg-ville'); if (v) v.textContent = C.VILLE || ''
})()

function setBtn(html, disabled) {
  const btn = document.getElementById('loginBtn')
  btn.innerHTML       = html
  btn.style.pointerEvents = disabled ? 'none' : ''
}

const SPINNER = `
  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" style="animation:spin .8s linear infinite;flex-shrink:0">
    <circle cx="12" cy="12" r="10" stroke="rgba(0,0,0,.15)" stroke-width="3"/>
    <path d="M12 2a10 10 0 0 1 10 10" stroke="#4285F4" stroke-width="3" stroke-linecap="round"/>
  </svg>`

const GOOGLE_ICON = `
  <svg width="20" height="20" viewBox="0 0 18 18" style="flex-shrink:0">
    <path fill="#4285F4" d="M17.64 9.2c0-.637-.057-1.251-.164-1.84H9v3.481h4.844c-.209 1.125-.843 2.078-1.796 2.717v2.258h2.908c1.702-1.567 2.684-3.875 2.684-6.615z"/>
    <path fill="#34A853" d="M9 18c2.43 0 4.467-.806 5.956-2.18l-2.908-2.259c-.806.54-1.837.86-3.048.86-2.344 0-4.328-1.584-5.036-3.711H.957v2.332C2.438 15.983 5.482 18 9 18z"/>
    <path fill="#FBBC05" d="M3.964 10.71c-.18-.54-.282-1.117-.282-1.71s.102-1.17.282-1.71V4.958H.957C.347 6.173 0 7.548 0 9s.348 2.827.957 4.042l3.007-2.332z"/>
    <path fill="#EA4335" d="M9 3.58c1.321 0 2.508.454 3.44 1.345l2.582-2.58C13.463.891 11.426 0 9 0 5.482 0 2.438 2.017.957 4.958L3.964 6.29C4.672 4.163 6.656 3.58 9 3.58z"/>
  </svg>
  Se connecter avec Google`

async function handleLogin(e) {
  e.preventDefault()
  const API = (window.ADMIN_CONFIG && window.ADMIN_CONFIG.API_URL) || 'http://localhost:5000'

  setBtn(SPINNER + ' Connexion en cours…', true)

  // Réveiller le backend avant de lancer l'OAuth
  // Si Render dort, ce ping déclenche le réveil côté serveur
  // On attend jusqu'à 30s (10 tentatives × 3s)
  let ok = false
  for (let i = 0; i < 10; i++) {
    try {
      if (i > 0) setBtn(SPINNER + ` Réveil du serveur… (${i * 3}s)`, true)
      const res = await fetch(API + '/api/health', { signal: AbortSignal.timeout(5000) })
      if (res.ok) { ok = true; break }
    } catch(err) {}
    await new Promise(r => setTimeout(r, 3000))
  }

  if (!ok) {
    // Le backend ne répond toujours pas — on tente quand même le OAuth
    setBtn(SPINNER + ' Redirection…', true)
  }

  window.location.href = API + '/api/auth/google'
}
